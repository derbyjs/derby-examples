...
var EventEmitter, Readable, arraydiff, assert, ot, redisLib;
...
exports.client = function(snapshotDb, redis, extraDbs) {
  var client, getDocOpChannel, getOpLogKey, nextStreamId, prefixChannel, redisDb, redisSubmit, streams;
  console.log('CREATE LIVEDB', redis)
  if (redis == null) {
  ...
  return client = {
    ...
    */
        ...
        return _this.fetch(cName, docName, function(err, snapshot) {
          ...
          trySubmit = function() {
            return redisSubmit(cName, docName, opData, function(err, result) {
              var d, db, name, old, oldOpData, _i, _len;
              if (err) {
                return typeof callback === "function" ? callback(err) : void 0;
              }
              ...
              for (name in extraDbs) {
                ...
                if (typeof db.submit === "function") {
                  db.submit(cName, docName, opData, snapshot, function(err) {
                    if (err) {
    ...
    _subscribeChannels: function(channels, callback) {
      var c, i, open, redisObserver, stream, _i, _len;
      stream = new Readable({
        objectMode: true
      });
      ...
      redisObserver.on('message', function(_channel, msg) {
        var data;
        if (!open) {
          return;
        }
        ...
        if (Array.isArray(channels)) {
          data.channel = _channel.slice(_channel.indexOf(' ') + 1);
        }
    ...
    queryFetch: function(cName, query, opts, callback) {
      ...
      return db.query(cName, query, function(err, results) {
        if (err) {
          return callback(err);
        } else if (Array.isArray(results)) {
          return callback(null, results);
        } else {
          return callback(null, results.results, results.extra);
        }
      });
    },
    query: function(index, query, opts, callback) {
      ...
      return this._subscribeChannels(channels, function(err, stream) {
        ...
        return db.query(index, query, function(err, results) {
          var d, docIdx, emitter, f, i, _i, _len;
          if (err) {
            stream.destroy();
            return callback(err);
          }
          ...
          (f = function() {
            ...
            while (d = stream.read()) {
              _results.push((function(d) {
                ...
                if (modifies === void 0) {
                  if (poll) {
                    return db.query(index, query, function(err, newResults) {
                      var diff, r, _j, _k, _len1, _len2;
                      if (err) {
                    ...
                    return db.queryDoc(index, d.c, d.docName, query, function(err, result) {
