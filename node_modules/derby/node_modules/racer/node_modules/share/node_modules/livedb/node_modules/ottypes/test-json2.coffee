...
genTests = (type) ->
  type.generateRandomOp = (data) ->
    ...
    op = while randomReal() < pct
      pct *= 0.6 # Exponentially prefer small ops

      # Pick a random object in the document operate on.
      {path, parent, key, operand} = randomLocation container

      if randomReal() < 0.4 and parent != container
        # Move the item to another random place in the object tree
        loop
          {path2, parent2, key2, operand2} = randomLocation container
          # It doesn't make sense if path2 is a parent of path1.
          break unless isParent path2, path1

        parent2[key2] = parent[key]
        ...
        # The path might be the same, but it should be fine.
        {m:path, to:newPath}

      else if randomReal() < 0.3 and parent != container and Array.isArray(parent)
        # List move
