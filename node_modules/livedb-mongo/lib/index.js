...
LiveDbMongo.prototype.query = function(cName, inputQuery, callback) {
  if (this.closed) return callback('db already closed');
...
LiveDbMongo.prototype.queryDoc = function(index, cName, docName, inputQuery, callback) {
  if (this.closed) return callback('db already closed');
  var query = normalizeQuery(inputQuery);

  if (query.$query._id) {
    if (query.$query._id !== docName) return callback();
  } else {
