###
  These configurations are for demo purposes, and should not be used in production. Instead, set environment
  variables (eg, ~/.profile or `heroku config:set FACEBOOK_KEY=...`) and access them in server/index.js via
  process.env (eg, process.env.FACEBOOK_KEY)
###
