...
# setImmediate isn't defined in node < 0.10
immediate = setImmediate or process.nextTick
...
exports.wait = (amt, callback) ->
	...
	waitInternal = (amt) ->
		...
		if queue.length > 0 and now + amt >= queue[0][0]
			...
			fn()
			immediate -> waitInternal amt, callback
		else
			now += amt
			callback() if callback?

	immediate -> waitInternal amt

exports.waitAll = (callback) ->
	if queue.length == 0
		immediate callback if callback?
	else
